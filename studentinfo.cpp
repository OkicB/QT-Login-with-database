...
void StudentInfo::on_pushButton_load_clicked()
{
...
void StudentInfo::on_loadButton_clicked()
{
    //load
        QSqlQuery qry;
        qry.prepare("select * from Grupa1 where ID=3");
        if(qry.exec()){
            while(qry.next()){
                ui->lineEdit_r1->setText(qry.value(0).toString());
                ui->lineEdit_r2->setText(qry.value(1).toString());
                ui->lineEdit_r3->setText(qry.value(2).toString());
                ui->lineEdit_r4->setText(qry.value(3).toString());
                ui->lineEdit_r5->setText(qry.value(4).toString());
            }
        }else{
            QMessageBox::critical(this,tr("error::"),qry.lastError().text());
        }
